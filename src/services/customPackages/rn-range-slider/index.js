...
const Slider = ({
  ...
  const [notchMargin, setNotchMargin] = useState(true);
  const [label, setLabel] = useState(true);
  const [active, setActive] = useState({
    lowStatus: true,
  });

  const { lowStatus } = active;
  ...
  useEffect(() => {
    updateThumbs();
  }, [updateThumbs, isPressed, lowStatus]);
      ...
      PanResponder.create({
        ...
        onPanResponderGrant: ({ nativeEvent }, gestureState) => {
          ...
          const handlePositionChange = (positionInView) => {
            // console.log(
            //   `gestureStateRef.current.lastValue,  gestureStateRef.current.lastPosition`,
            //   gestureStateRef.current.lastValue,
            //   gestureStateRef.current.lastPosition,
            //   gestureStateRef.current.isLow,
            // );
            ...
            setActive((prevState) => {
              return {
                ...prevState,
                lowStatus: gestureStateRef.current.isLow,
              };
            });
  ...
  return (
    <View {...restProps}>
      <View {...labelContainerProps}>
        {/* <View style={{ marginLeft: label ? 100 : 0 }}>{labelView}</View> */}
        {labelView}
        <View style={{ marginLeft: notchMargin ? 12 : 0 }}>{notchView}</View>
      </View>
      <View onLayout={handleContainerLayout} style={styles.controlsContainer}>
        ...
        <Animated.View style={[lowStyles]} onLayout={handleThumbLayout}>
          <View
            style={{
              backgroundColor: lowStatus ? '#3FA9F5' : '#8F93A2',
              padding: 3,
              borderRadius: 360,
            }}>
            {lowThumb}
